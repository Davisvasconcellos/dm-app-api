...
async function run() {
  ...
  await sequelize.transaction(async (t) => {
    const idxName = 'financial_categories_store_status_idx';
    ...
    await sequelize.query(
      `
        ...
        FROM stores s
        WHERE fc.store_id_code IS NULL
          AND fc.store_id IS NOT NULL
          AND s.id = fc.store_id
      `,
      { transaction: t }
    );
    ...
    await sequelize.query(
      `CREATE INDEX IF NOT EXISTS ${idxName} ON ${tableName} (store_id, status);`,
      { transaction: t }
    );
  });
}
...
run()
  ...
  .catch(async (error) => {
    console.error('Erro ao ajustar tipo de store_id em financial_categories:', error);
    try {
      await sequelize.close();
    } catch (e) {
    }
