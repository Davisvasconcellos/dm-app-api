...
async function run() {
  ...
  await ensureUniqueIndex(tableName, 'financial_cost_centers_id_code_uq', ['id_code']);
...
run()
  ...
  .catch(async (error) => {
    console.error('Erro ao ajustar schema de financial_cost_centers:', error);
    try {
      await sequelize.close();
    } catch (e) {
    }
