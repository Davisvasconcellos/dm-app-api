...
async function run() {
  ...
  await sequelize.query(`CREATE INDEX IF NOT EXISTS financial_cost_centers_store_status_idx ON ${tableName} (store_id, status);`);
}
...
run()
  ...
  .catch(async (error) => {
    console.error('Erro ao ajustar coluna status de financial_cost_centers:', error);
    try {
      await sequelize.close();
    } catch (e) {
    }
