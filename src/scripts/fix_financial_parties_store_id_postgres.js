...
async function run() {
  ...
  await sequelize.transaction(async (t) => {
    const idxNames = [
      'financial_parties_store_status_idx',
      'financial_parties_store_id_idx'
    ];
    ...
    await sequelize.query(
      `
        ...
        FROM stores s
        WHERE fp.store_id_code IS NULL
          AND fp.store_id IS NOT NULL
          AND s.id = fp.store_id
      `,
      { transaction: t }
    );
    ...
    await sequelize.query(
      `CREATE INDEX IF NOT EXISTS financial_parties_store_id_idx ON ${tableName} (store_id);`,
      { transaction: t }
    );
    ...
    if (statusInfo) {
      await sequelize.query(
        `CREATE INDEX IF NOT EXISTS financial_parties_store_status_idx ON ${tableName} (store_id, status);`,
        { transaction: t }
      );
    }
  });
...
run()
  ...
  .catch(async (error) => {
    console.error('Erro ao ajustar tipo de store_id em financial_parties:', error);
    try {
      await sequelize.close();
    } catch (e) {
    }
